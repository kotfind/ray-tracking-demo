...
struct Material {
    Material(const Vec3f &c, const float &spec) : diffuse_colour(c), specular_exponent(spec) {}
    Material() : diffuse_colour(), specular_exponent() {}
...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    return material.diffuse_colour * diffuse_light_intensity * powf(specular_light_intensity + 0.3, 1 / 8.);
}
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    for (size_t j = 0; j < img_height; ++j) {
        for (size_t i = 0; i < img_width; ++i) {
            ...
            img[i][j] = cast_ray(Vec3f(0, 0, 0), dir, spheres, lights);
        }
    }
...
int main() {
    Material m1(Vec3f(0.4, 0.4, 0.3), 50.);
    Material m2(Vec3f(0.3, 0.1, 0.1), 10.);
